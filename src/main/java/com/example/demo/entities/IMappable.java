...
public interface IMappable {
    ...
    // metodo che prende in input una mappa, chiamerà un set per volta e confronterà
    // il
    // nome della proprietà dell'oggetto con il nome di ogni chiave della mappa
    // se c'è corrispondenza allora aasegnarà a quella proprietà il valore
    // assocciato alla chiave con lo stesso nome
    // passare i dati dati di una mmap alle proprietà di un oggetto in modo che
    // siano valorizzate in automatico
    default void fromMap(Map<String, String> map) {
        // prendo un set per volta
        // this.getClass().getMethods() -> questa istruzione ritorna un array di oggetti
        // di tipo Method
        // cioè un array che contiene tutti i metodi dichiarati nella classe modello
        // dell'oggetto(this) che sta
        // invocando il metodo fromMap()
        for (Method m : this.getClass().getMethods()) {
            // controllo che il metodo preso in considerazione nell'iterazione sia un set
            if (m.getName().startsWith("set") && m.getParameterCount() == 1) {
                // setNome
                // dal nome del metodo tolgo il valore set e mettendo in minuscolo il primo
                // carattere ottengo il nome della proprietà
                // setNome -> Nome
                ...
                // del metodo e togliendo i primi 3 caratteri quindi togliendo set al nome del
                // metodo
                // ad es. con il metodo setNome avrò salvato in nomeProp = "Nome"; N[0], o[1],
                // m[2], e[3];
                // Character.toLowerCase(nomeProprieta.charAt(0)) -> "n"
                // nomeProprieta.substring(1) -> "Nome" -> sottostringa a partire dall'indice 1
                // -> quindi "ome"
                ...
                // Character.toLowerCase(nomeProprieta.charAt(4).toLowerCase) -> prendo la "N"
                // in dataNascita e la metto minuscola
                // nomeProprieta =nomeProprieta.substring(0, 4) + "_" +
                // Character.toLowerCase(nomeProprieta.charAt(4).toLowerCase) -> "data_n"
                // +nomeProprieta.substring(5); -> "dataNascita" prendo tutti i char dall'indice
                // 5 in poi "ascita"
                // nomeProprieta =nomeProprieta.substring(0, 4) + "_" +
                // Character.toLowerCase(nomeProprieta.charAt(4))
                // +nomeProprieta.substring(5); -> nomeProprieta = "data_nascita"

                // confronto il nome della proprietà dell'oggetto con il nome delle chiavi della
                // mappa
                // se esiste una chiave con lo stesso nome del nome della proprietà allora
                // prendo il valore associato e lo passo
                ...
                if (map.containsKey(nomeProprieta)) {
                    ...
                    String valoreAssociato = map.get(nomeProprieta); // in valore associato se uso l'esempio della
                                                                     // proprietà nome
                    // sto prendendo il nome di un dipendente dalla mappa letta dal db e lo salvo in
                    // una variabile
                    ...
                    // m.getParameters() restituisce un array con all'interno tutti i parametri
                    // presi in input dal metodo
                    // m.getParameters()[0].getType() restituisce il tipo (come classe) del primo
                    // parametro dell'array
                    ...
                    try {
                        switch (tipoParametro) {
                            case "string":
                                ...
                                // se tutto va bene alla riga 69 ho preso un valore della mappa e l'ho passato
                                // ad un set quindi settato il valore
                            ...
                            case "double":
                                ...
                                break;
                            case "localdate":
                                // valoreAssociato conterrà una data ma in formato stringa, per passarla al set
                                // che prende in input oggetti di
    ...
    // dove le chiavi saranno i nomi delle proprietà e i valori i valori ad esse
    // associati
